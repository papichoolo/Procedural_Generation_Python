...
layers1=np.array(layers, dtype=object)
...
class TerrainGenerator:
    ...
    def assign_colors(self, layers1, noise_array, sea_level):
        altitudes = (layers1[:, 2] + sea_level).astype(int)
        colors = np.array([np.array([*color], dtype=np.uint8) for color in layers1[:, 1]])
...
city_points = [(100, 100), (200, 200), (300, 300)]
...
color_array = terrain_generator.assign_colors(layers1, noise_array, sea_level)
...
while running:
    ...
    def draw_city_points(city_points):
        for point in city_points:
            pygame.draw.circle(screen, (255, 0, 0), (point[0] * zoom_level + offset_x, point[1] * zoom_level + offset_y), 5)
            # Adjust the size or color of circles as needed

    # Draw city points on the screen
    draw_city_points(city_points)
